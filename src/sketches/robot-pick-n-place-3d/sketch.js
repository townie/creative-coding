/**
 * 6-DOF ROBOT ARM SIMULATION v6.0 - JOINT LIMITS & PHYSICS
 * Features:
 * - 6 Degrees of Freedom
 * - Impulse-based Physics Engine
 * - True Mesh-based Collision
 * - Lidar/Vision System
 * - UI: Control, Calibration, Joint Limits
 */
  ...
  handlePress() {
    ...
    for (let item of this.world.items) {
      ...
      if (delta > 0) {
        ...
        if (t > 0 && t < minT) { minT = t; closest = item; }
      }
    }
    if (closest) {
...
class Robot {
  constructor(world) {
    ...
    this.limits = {
      base: { min: -PI, max: PI },
      shoulder: { min: -HALF_PI, max: HALF_PI },
      elbow: { min: 0, max: PI },
      forearmRoll: { min: -PI, max: PI },
      wristPitch: { min: -PI, max: PI },
      wristRoll: { min: -PI, max: PI }
    };
  ...
  solveIK(target, targetWristRoll = 0) {
    ...
    // Check validity against limits
    let limitCheck = true;
    for (let key in j) {
      let val = j[key];
      let min = this.limits[key].min;
      let max = this.limits[key].max;
      let clamped = constrain(val, min, max);
      if (Math.abs(clamped - val) > 0.001) {
        limitCheck = false;
        j[key] = clamped; // Clamp to limit
      }
    }

    this.hasIKSolution = valid && limitCheck;
    return j;
  }
  ...
  solveFK() {
    ...
    let baseT = t.copy(); baseT.translate(0, 10, 0);
    let leftT = baseT.copy(); leftT.translate(-this.gripWidth/2 - 4, 25, 0);
    let rightT = baseT.copy(); rightT.translate(this.gripWidth/2 + 4, 25, 0);
    ...
    let endPos = t.transformPoint(0, 72, 0);
  ...
  update() {
    ...
    if (this.isManual) {
      // Enforce limits during manual control
      for(let key in this.joints) {
        this.joints[key] = constrain(this.joints[key], this.limits[key].min, this.limits[key].max);
      }
      this.solveFK();
      return;
    }
  ...
  processMotion() {
    if (this.waypointQueue.length > 0) {
      ...
      for (let key in this.joints) {
        let val = lerp(this.currentStartJoints[key], wp.joints[key], ease);
        // Safety clamp during motion
        this.joints[key] = constrain(val, this.limits[key].min, this.limits[key].max);
      }
      ...
      if (t >= 1.0) {
        ...
        if (finishedWp.action === "GRIP_CHECK") {
          if (this.isGripped) this.gripFailCount = 0;
          else { this.handleGripFailure(); return; }
        }
      }
    }
  }
  ...
  handleGripFailure() {
    ...
    if (!this.currentTargetBase) { this.state = "IDLE"; return; }
    this.waypointQueue = [];
    this.currentStartJoints = null;
    this.currentStartGrip = this.gripWidth;
    let retryOffset = this.getRetryOffset();
    this.planPickSequence({ pos: this.currentTargetBase.pos.copy(), rot: this.currentTargetBase.rot }, retryOffset);
    this.state = "MOVING";
  }

  resetAfterGripFailures() {
    if (this.heldItem) { this.heldItem.grippedBy = null; this.heldItem = null; }
  ...
  planPickSequence(targetData, offset = null) {
    ...
    drop.x += random(-30, 30); drop.z += random(-30, 30);
    ...
    this.addWaypoint(createVector(p.x, p.y, p.z), 25, 400, orientation);
    this.addWaypoint(createVector(p.x, p.y, p.z), 25, 200, orientation, "GRIP_CHECK");
    ...
    this.addWaypoint(createVector(drop.x, CONFIG.floorY - 140, drop.z), 40, 300, dropRot);
  ...
  display() {
    ...
    if (this.state === "CALIBRATING_ADJUST") {
      push(); noFill(); stroke(0, 255, 0, 100); strokeWeight(1);
      this.drawRobotModel(true); pop();
    }
  ...
  drawRobotModel(isGhost) {
    ...
    if(!isGhost) fill(40); cylinder(50, 20); translate(0, -CONFIG.baseHeight/2, 0);
    rotateY(j.base + o.base);
    if(!isGhost) fill(255, 120, 0); cylinder(40, CONFIG.baseHeight); translate(0, -CONFIG.baseHeight/2, 0);
    ...
    if(!isGhost) fill(180);
    push(); translate(-gw - 4, 25, 0); box(8, 50, 25); pop();
    push(); translate(gw + 4, 25, 0); box(8, 50, 25); pop();
  }
}
...
class PhysicsWorld {
  ...
  initEnvironment() {
    ...
    for(let i=0; i<4; i++) {
      this.items.push(new Item(-300 + random(-25, 25), CONFIG.floorY - 100 - i*50, random(-25, 25)));
    }
  }
  spawnBlock() {
  ...
  resetBlocks(count = 4) {
    this.items = [];
    for (let i = 0; i < count; i++) this.spawnBlock();
  }
  update(dt, robot) {
    for (let item of this.items) {
      ...
      item.pos.add(p5.Vector.mult(item.vel, dtStep));
      item.rot += item.angVel * dtStep;
      item.angVel *= Math.pow(0.98, dtStep);
  ...
  applyRobotCollisions(robot) {
    ...
    for (let item of this.items) {
      ...
      for (let part of robot.gripperParts) {
        ...
        if (distSq < (item.size/2)**2) {
          ...
          if (dist < 0.0001) normalLocal = createVector(0,1,0);
          ...
          if (vn < 0) {
            ...
            item.vel.add(p5.Vector.mult(tangent, -vt * 0.5));
          }
          contactCount++;
    ...
    if (robot.heldItem) {
      if (robot.gripWidth > robot.heldItem.size + 2) {
        ...
        robot.heldItem.vel.add(p5.Vector.sub(robot.effector, robot.prevEffector).mult(0.5));
        robot.heldItem = null;
      } else {
  ...
  resolveItemCollision(a, b) {
    ...
    if (distSq < minD * minD && distSq > 0.0001) {
      ...
      let j = -(1 + CONFIG.restitution) * velAlongNormal;
      ...
      tangent.normalize();
      let jt = -p5.Vector.dot(rv, tangent) * 0.5;
...
class Item {
  ...
  display() {
    ...
    if (!this.grippedBy && this.pos.y < CONFIG.floorY - this.size) {
      push(); translate(0, CONFIG.floorY - this.pos.y - 1, 0); rotateX(HALF_PI);
      fill(0, 100); noStroke(); circle(0, 0, this.size * 1.2); pop();
    }
...
class TopHUD {
  ...
  spawnBlock() { this.world.spawnBlock(); }
  update() {
...
class VisionGUI {
  constructor(scannerRef) {
    ...
    // Attach the graphics canvas to the content div
    this.pg.parent(this.content);
    this.pg.style('display', 'block');
  }
  ...
  display() {
    // Always render the buffer - it's attached to the DOM and will show when modal is open
    this.renderBuffer();
  }
}
...
class ControlPanel {
  constructor(robotRef) {
    ...
    this.tabBtnCalib = createButton('CALIB').class('tab-btn').parent(tabHeader).mousePressed(() => this.switchTab('calibration'));
    this.tabBtnLimits = createButton('LIMITS').class('tab-btn').parent(tabHeader).mousePressed(() => this.switchTab('limits'));
    ...
    this.contentLimits = createDiv('').class('tab-content').parent(this.panel);
    createDiv('JOINT LIMITS (RAD)').class('panel-header').parent(this.contentLimits);
    this.limitSliders = {};
    const jointNames = ['base', 'shoulder', 'elbow', 'forearmRoll', 'wristPitch', 'wristRoll'];
    jointNames.forEach(j => {
      let g = createDiv('').class('control-group').parent(this.contentLimits);
      createDiv(j.toUpperCase()).class('control-label').parent(g).style('color', '#00ffc8');
      createDiv('MIN').class('control-label').parent(g);
      let sMin = createSlider(-PI*1.5, PI*1.5, this.robot.limits[j].min, 0.01).parent(g);
      createDiv('MAX').class('control-label').parent(g);
      let sMax = createSlider(-PI*1.5, PI*1.5, this.robot.limits[j].max, 0.01).parent(g);
      this.limitSliders[j] = { min: sMin, max: sMax };
    });
  }
  ...
  switchTab(tabName) {
    ...
    this.tabBtnControl.removeClass('active'); this.tabBtnCalib.removeClass('active'); this.tabBtnLimits.removeClass('active');
    this.contentControl.removeClass('active'); this.contentCalib.removeClass('active'); this.contentLimits.removeClass('active');

    if(tabName === 'control') { this.tabBtnControl.addClass('active'); this.contentControl.addClass('active'); }
    else if(tabName === 'calibration') { this.tabBtnCalib.addClass('active'); this.contentCalib.addClass('active'); }
    else if(tabName === 'limits') { this.tabBtnLimits.addClass('active'); this.contentLimits.addClass('active'); }
  }
  ...
  sync() {
    ...
    if(this.activeTab === 'limits') {
      for (let j in this.limitSliders) {
        let minVal = this.limitSliders[j].min.value();
        let maxVal = this.limitSliders[j].max.value();
        // Ensure min <= max
        if (minVal > maxVal) { minVal = maxVal; this.limitSliders[j].min.value(minVal); }
        this.robot.limits[j].min = minVal;
        this.robot.limits[j].max = maxVal;
      }
    }
  }
}
...
function injectStyles() {
  let css = `
    ...
    .vision-content { width: 240px; height: 240px; margin: 10px; background: #000; }
    .btn-vision-toggle {
